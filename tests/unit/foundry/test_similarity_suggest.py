...
class TestSuggest:
    ...
    def test_case_insensitive_exact_match(self):
        ...
        suggestions = similarity.suggest(
            "DOCSCRIBE", ["docscribe", "Docscribe", "DocScribe"], min_score=0.9, normalize_text=True
        )
    ...
    def test_no_matches_above_threshold(self):
        ...
        suggestions = similarity.suggest("xyz", ["abc", "def", "ghi"], min_score=0.6)
    ...
    def test_max_suggestions_limit(self):
        ...
        suggestions = similarity.suggest(
            "test", ["test1", "test2", "test3", "test4", "test5"], min_score=0.6, max_suggestions=2
        )
    ...
    def test_empty_input(self):
        ...
        suggestions = similarity.suggest("", ["test", "hello", "world"], min_score=0.6)

        assert len(suggestions) == 0

    def test_empty_candidates(self):
        """Test empty candidates returns empty list."""
        suggestions = similarity.suggest("test", [], min_score=0.6)
