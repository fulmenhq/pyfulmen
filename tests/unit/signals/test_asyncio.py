...
class TestAsyncSignalHandler:
    ...
    def test_call_async_handler_with_loop(self):
        ...
        async def test_in_loop():
            ...
            assert asyncio.isfuture(result) or asyncio.istask(result)
            return result
        ...
        try:
            task = loop.run_until_complete(test_in_loop())
...
class TestGlobalFunctions:
    ...
    def test_register_with_asyncio_if_available_success(self, mock_integration):
        ...
        assert result is True
        mock_integration.register_async_handler.assert_called_once_with(
            stdlib_signal.SIGTERM, handler
        )
...
class TestCreateAsyncSafeHandler:
    ...
    def test_create_async_safe_handler_with_asyncio_and_async_handler(
        self, mock_iscoro, mock_is_async
    ):
    ...
    @patch('pyfulmen.signals._asyncio.is_asyncio_available')
    @patch('pyfulmen.signals._asyncio.asyncio.iscoroutinefunction')
    def test_create_async_safe_handler_with_asyncio_and_sync_handler(
        self, mock_iscoro, mock_is_async
    ):
    ...
    @patch('pyfulmen.signals._asyncio.asyncio.iscoroutinefunction')
    @patch('asyncio.run')
    def test_create_async_safe_handler_no_asyncio_async_handler(
        self, mock_run, mock_iscoro, mock_is_async
    ):
    ...
    @patch('pyfulmen.signals._asyncio.is_asyncio_available')
    @patch('pyfulmen.signals._asyncio.asyncio.iscoroutinefunction')
    def test_create_async_safe_handler_no_asyncio_sync_handler(
        self, mock_iscoro, mock_is_async
    ):
