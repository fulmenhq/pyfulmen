...
class TestDualEmissionIntegration:
    ...
    def test_dual_emission_end_to_end(self):
        ...
        with patch.dict(os.environ, {"PYFULMEN_TELEMETRY_ALIAS": "true"}):
            ...
            counter = create_aliased_counter(
                registry,
                canonical_name="test_requests_total",
                legacy_name="legacy_requests_total"
            )
    ...
    def test_environment_variable_runtime_changes(self):
        ...
        with patch.dict(os.environ, {"PYFULMEN_TELEMETRY_ALIAS": "false"}):
            counter1 = create_aliased_counter(
                registry,
                canonical_name="test_metric_1",
                legacy_name="legacy_metric_1"
            )
            counter1.inc()
        ...
        with patch.dict(os.environ, {"PYFULMEN_TELEMETRY_ALIAS": "true"}):
            counter2 = create_aliased_counter(
                registry,
                canonical_name="test_metric_2",
                legacy_name="legacy_metric_2"
            )
            counter2.inc()
    ...
    def test_mixed_metric_types_dual_emission(self):
        ...
        with patch.dict(os.environ, {"PYFULMEN_TELEMETRY_ALIAS": "true"}):
            ...
            counter = create_aliased_counter(
                registry,
                canonical_name="api_requests_total",
                legacy_name="legacy_api_requests_total"
            )
            
            from pyfulmen.telemetry import create_aliased_gauge
            gauge = create_aliased_gauge(
                registry,
                canonical_name="memory_usage_bytes",
                legacy_name="legacy_memory_usage_bytes"
            )
