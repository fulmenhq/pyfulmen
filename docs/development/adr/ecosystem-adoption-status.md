...
| [ADR-0008](../../crucible-py/architecture/decisions/ADR-0008-helper-library-instrumentation-patterns.md) | Helper Library Instrumentation Patterns | **implemented** (30) | Module-level helpers on the global registry are the sanctioned library pattern. Known exceptions: signals records the unregistered dotted name `fulmen.signal.unsupported` (taxonomy gap reported upstream); fulhash retains a duration histogram beyond the ADR's counter-only guidance | [ADR-0008](ADR-0008-global-metric-registry-singleton.md)    |
