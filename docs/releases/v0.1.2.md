...
- **Version Propagation**: Policy-driven VERSION → pyproject.toml sync automation
...
1. **validate_country_code(code)**: Three-lookup strategy
   - Tries Alpha-2 → Alpha-3 → Numeric (with zero-padding)
   - Case-insensitive: `"us"` → `"US"`, `"usa"` → `"USA"`
...
4. **get_country_by_numeric(numeric)**: Lookup by numeric code
   - Automatic zero-padding: `"76"` → `"076"` (Brazil)
   - Returns `Country` object or `None`
...
├── README.md           # Index and promotion process
├── template.md         # ADR template
├── ADR-0001-*.md      # FulmenCatalogModel populate_by_name=True
├── ADR-0002-*.md      # validate_country_code() Three-Lookup Strategy
└── ADR-0003-*.md      # Country Catalog Preview Status
```
...
- **Rationale**: Enables camelCase → snake_case mapping for Crucible integration
- **Impact**: Python-specific Pydantic configuration detail

**ADR-0002**: validate_country_code() Three-Lookup Strategy
- **Decision**: Alpha-2 → Alpha-3 → Numeric fallback sequence
...
**Workflow**: VERSION file → goneat version propagate → pyproject.toml [project] section
...
- ✅ Country Code API: 100% gofulmen v0.1.1 parity
- ✅ MIME Detection API: 100% gofulmen v0.1.1 parity (28/28 features)
- ✅ Golden Fixtures: All 8 fixtures passing
...
validate_country_code("76")     # True (Numeric with zero-padding → "076")
...
# Bump patch version (0.1.2 → 0.1.3, auto-propagates)
make version-bump-patch
